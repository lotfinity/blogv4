...
document.addEventListener('click', function(e) {
    if (e.target.closest('.item_img')) {
        const postItem = e.target.closest('.item');
        const postId = postItem.dataset.postId; // You'll need to add data-post-id to your items
        loadPostDetails(postId);
    }
});

function loadPostDetails(postId) {
    // Example AJAX call - modify according to your backend
    fetch(`/api/posts/${postId}/`)
        .then(response => response.json())
        .then(data => {
            const modalContent = document.querySelector('#postDetailModal .post-media-container');
            ...
            // Update other modal sections
            document.querySelector('.post-comments').innerHTML = data.comments;
            document.querySelector('.post-actions').innerHTML = `
                <div class="d-flex justify-content-between">
                    <div class="like-btn">
                        ${data.is_liked ? '<img src="heart-filled.png">' : '<img src="heart-outline.png">'}
                    </div>
                    <div class="save-btn">
                        ${data.is_saved ? '<img src="bookmark-filled.png">' : '<img src="bookmark-outline.png">'}
                    </div>
                </div>
            `;
...
document.querySelector('#postDetailModal').addEventListener('hidden.bs.modal', function() {
    // Pause any playing videos when modal closes
    const videos = document.querySelectorAll('#postDetailModal video');
...
if (image_profile) {
    // Your existing code that uses image_profile
}
// ...existing code...
...
function loadPosts() {
    ...
    posts.forEach((post, index) => {
        if (index < 30) {
            // Load immediately for the first 30 posts
            loadPost(post);
        } else {
...
function loadPost(post) {
    // Your existing code to load a post
}
