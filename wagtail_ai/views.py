...
def _process_backend_request(
    ...
    try:
        ...
        logger.debug(f"Sending request to AI backend with pre_prompt: {pre_prompt}, context: {context[:100]}...")

        response = ai_backend.prompt_with_context(pre_prompt=pre_prompt, context=context)

        # Log the response details
        logger.debug(f"Received response from AI backend: {response.text()[:100]}...")
    except Exception as e:
        logger.error(f"Error processing request with pre_prompt: {pre_prompt}, context: {context[:100]}...")
...
def _replace_handler(*, prompt: Prompt, text: str) -> str:
    ...
    for split in texts:
        # Log the split texts
        logger.debug(f"Processing text split: {split[:100]}...")
        ...
        # Log the replaced message
        logger.debug(f"Replaced text: {message[:100]}...")
...
def _append_handler(*, prompt: Prompt, text: str) -> str:
    ...
    # Log the text length
    logger.debug(f"Original text length: {length_calculator.get_splitter_length(text)}")

    if length_calculator.get_splitter_length(text) > ai_backend.config.token_limit:
        logger.error("Text length exceeds token limit")
    ...
    # Log the appended message
    logger.debug(f"Appended text: {message[:100]}...")
...
def text_completion(request) -> JsonResponse:
    ...
    try:
        ...
        logger.error(f"AIHandlerException: {str(e)}")
...
def describe_image(request) -> JsonResponse:
    ...
    if not user_has_permission_for_image(request.user, image):
        logger.error("Access denied for image description.")
    ...
    try:
        ...
        logger.error("No backend is configured for image description.")
